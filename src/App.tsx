...
function App() {
  ...
  // Extrae el ID de YouTube (soporta formato /shorts/)
  ...
  return (
    <>
      ...
      {/* TopNavBar */}
      <nav className="fixed top-0 w-full z-50 bg-[#131314]/80 backdrop-blur-xl flex justify-between items-center px-8 h-20 shadow-none">
        <div className="flex items-center gap-12">
          ...
          <div className="hidden md:flex gap-8 items-center font-['Montserrat'] tracking-tight font-bold">
            <a className="text-[#DDDADB] hover:text-[#F07D00] transition-colors duration-300" href="#">Noticias</a>
            <a className="text-[#C13535] border-b-2 border-[#C13535] pb-1" href="#">Programas</a>
            <a className="text-[#DDDADB] hover:text-[#F07D00] transition-colors duration-300" href="#">Archivo</a>
          </div>
        </div>
        <div className="flex items-center gap-6">
          <div className="relative">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#DDDADB]/40">search</span>
            <input 
              type="text" 
              placeholder="Buscar programas o videos..."
              className="bg-surface-container-low border-none rounded-full py-2 pl-10 pr-4 text-sm text-[#DDDADB] w-48 md:w-64 focus:ring-2 focus:ring-[#F07D00]/50 transition-all"
        ...
        <section className="relative h-[870px] w-full flex items-center overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-8 w-full grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              ...
              <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-on-surface leading-none text-shadow-editorial">
                {featuredVideo ? (
                  <span dangerouslySetInnerHTML={{ __html: featuredVideo.title.replace(' ', '<br />') }} />
                ) : (
                  <>VOCES DEL <br /><span className="text-[#C13535]">MAÑANA</span></>
                )}
              </h1>
              <p className="text-lg text-[#DDDADB]/80 max-w-lg leading-relaxed font-medium">
                {featuredVideo 
              ...
              <div className="flex items-center gap-4 pt-4">
                ...
                <button className="bg-surface-container/50 backdrop-blur-md text-on-surface px-8 py-4 rounded-full font-bold border border-outline-variant/30 hover:bg-surface-container transition-all">
                  MÁS INFO
                </button>
              </div>
            </div>
            ...
            <div className="hidden md:block relative group cursor-pointer" onClick={() => featuredVideo && navigate(`/watch/${featuredVideo.id}`)}>
              ...
              {nextVideo && (
                <div onClick={(e) => { e.stopPropagation(); navigate(`/watch/${nextVideo.id}`); }} className="absolute -bottom-6 -right-6 glass-card p-4 rounded-lg border border-outline-variant/20 hidden lg:block hover:scale-105 transition-transform cursor-pointer shadow-2xl z-20">
        ...
        <section className="py-24 bg-surface-container-low overflow-hidden">
          <div className="max-w-7xl mx-auto px-8">
            <div className="flex items-end justify-between mb-12">
              <div>
                <h2 className="text-3xl font-black tracking-tight text-[#C13535]">PROGRAMAS DESTACADOS</h2>
          ...
          <div className="max-w-7xl mx-auto px-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-16">
              <div className="max-w-xl">
                <h2 className="text-4xl font-black tracking-tighter text-on-surface">FORMATO <span className="text-[#F07D00]">VERTICAL</span></h2>
                <p className="text-[#DDDADB]/60 mt-2">Disfruta de nuestros Shorts y Reels. Un vistazo rápido al mejor contenido de Estudio Radio América.</p>
              </div>
                  ...
                  <button 
                    ...
                    className={`px-6 py-2 rounded-full font-bold text-sm transition-colors ${
                      activeCategory === category 
                        ...
                        : 'bg-surface-container-low text-[#DDDADB]/60 hover:bg-[#C13535]'
                    }`}
                  >
                    {category}
                  </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
              ...
              <div className="md:col-span-7 flex flex-col gap-6">
                <h3 className="text-xs font-bold text-[#C13535] tracking-widest uppercase mb-2 border-b border-outline-variant/20 pb-2">Feed de Shorts & Reels</h3>
                {shorts.length === 0 ? (
                  <p className="text-sm text-[#DDDADB]/40 py-4">No hay shorts en esta categoría.</p>
                ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {shorts.map((short) => (
                      <div key={short.id} className="flex flex-col gap-2">
                        {playingShortId === short.id ? (
                          <div className="aspect-[9/16] bg-surface-container-highest rounded-xl overflow-hidden relative shadow-lg group/player">
                            ...
                            {getYoutubeId(short.url) ? (
                              ...
                              <div className="w-full h-full relative z-10 flex flex-col items-center justify-center bg-surface-container-highest p-4 text-center">
                          ...
                          <div onClick={() => setPlayingShortId(short.id)} className="aspect-[9/16] relative rounded-xl overflow-hidden group cursor-pointer border border-outline-variant/10 shadow-lg group-hover:scale-105 transition-transform">
                        ...
                        <h4 className="text-xs font-bold text-[#DDDADB] line-clamp-2 px-1 hover:text-[#F07D00] cursor-pointer" onClick={() => !playingShortId && setPlayingShortId(short.id)}>{short.title}</h4>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="md:col-span-5 flex flex-col gap-6 border-l border-outline-variant/10 md:pl-12">
                <h3 className="text-xs font-bold text-[#C13535] tracking-widest uppercase mb-2 border-b border-outline-variant/20 pb-2">Archivo de Video Destacado</h3>
                
                {/* Featured Category Video */}
                {categoryFeaturedVideo && (
                  <div onClick={() => navigate(`/watch/${categoryFeaturedVideo.id}`)} className="w-full glass-card rounded-2xl overflow-hidden relative border border-outline-variant/10 group cursor-pointer mb-4">
                    ...
                    <div className="p-6">
                      ...
                      <h4 className="font-bold text-2xl leading-tight mb-2 group-hover:text-primary transition-colors">{categoryFeaturedVideo.title}</h4>
                      <p className="text-xs text-[#DDDADB]/60 line-clamp-3">{categoryFeaturedVideo.description}</p>
                    </div>
                  </div>
                )}
              </div>
        ...
        <section className="py-20 bg-surface-container">
          <div className="max-w-5xl mx-auto px-8 text-center space-y-8">
            <div className="inline-block p-4 rounded-full bg-[#C13535]/10 border border-[#C13535]/20 mb-4">
              <span className="material-symbols-outlined text-4xl text-[#C13535]" style={{ fontVariationSettings: "'FILL' 1" }}>mail</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-black tracking-tighter">ÚNETE A LA <span className="text-[#C13535]">COMUNIDAD</span></h2>
            <p className="text-xl text-[#DDDADB]/60 max-w-2xl mx-auto">Recibe todas las últimas novedades de tus programas favoritos directamente en tu correo. Entérate al instante cuando subamos nuevo contenido.</p>
            <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
              <input required type="email" className="flex-1 bg-surface-container-lowest border border-outline-variant/20 focus:ring-2 focus:ring-[#F07D00] rounded-full px-6 py-4 text-[#DDDADB] placeholder:text-[#DDDADB]/30" placeholder="Tu correo electrónico" />
            ...
            <p className="text-[10px] text-[#DDDADB]/40 uppercase tracking-widest">Al suscribirte aceptas nuestros términos y condiciones.</p>
          </div>
        </section>
      </main>
      ...
      <div className="fixed bottom-0 w-full z-50 px-4 pb-4">
        <div className="max-w-7xl mx-auto h-20 glass-nav rounded-2xl border-t-2 border-[#F07D00]/30 flex items-center justify-between px-6 shadow-2xl">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-lg overflow-hidden bg-white relative group flex items-center justify-center p-1">
              <img alt="Playing" className="w-full h-full object-contain" src="/logo_colors.png" />
            ...
            <div className="hidden sm:block">
              ...
              <p className="font-bold text-sm truncate max-w-[150px]">Estudio Radio América - Prime Time</p>
            </div>
          </div>
          
          <div className="flex items-center gap-8">
            <button className="text-[#DDDADB]/60 hover:text-on-surface transition-colors hidden md:block">
          ...
          <div className="flex items-center gap-6">
            <div className="hidden lg:flex items-center gap-3 group">
              <span className="material-symbols-outlined text-[#DDDADB]/60 text-lg">volume_up</span>
      ...
      {/* Footer */}
      <footer className="bg-[#131314] w-full border-t border-[#59413f]/15 pt-16 pb-32">
        <div className="max-w-7xl mx-auto px-8 flex flex-col items-center space-y-8">
          <span className="text-xl font-bold text-[#C13535]">Estudio Radio América</span>
          <div className="flex flex-wrap justify-center gap-8 font-['Montserrat'] text-sm text-[#DDDADB]">
            <a className="text-[#DDDADB]/60 hover:text-[#C13535] transition-colors opacity-100 hover:opacity-80" href="#">Privacidad</a>
            <a className="text-[#DDDADB]/60 hover:text-[#C13535] transition-colors opacity-100 hover:opacity-80" href="#">Términos</a>
            <a className="text-[#DDDADB]/60 hover:text-[#C13535] transition-colors opacity-100 hover:opacity-80" href="#">Contacto</a>
            <a className="text-[#DDDADB]/60 hover:text-[#C13535] transition-colors opacity-100 hover:opacity-80" href="#">Publicidad</a>
          </div>
          <div className="flex gap-6">
            <span className="material-symbols-outlined text-[#DDDADB]/40 cursor-pointer hover:text-[#C13535]">share</span>
            <span className="material-symbols-outlined text-[#DDDADB]/40 cursor-pointer hover:text-[#C13535]">language</span>
            <span className="material-symbols-outlined text-[#DDDADB]/40 cursor-pointer hover:text-[#C13535]">podcasts</span>
          </div>
          <p className="text-center font-['Montserrat'] text-sm text-[#DDDADB]/40">© 2024 Estudio Radio América. Todos los derechos reservados.</p>
        </div>
      </footer>
    </>
  )
