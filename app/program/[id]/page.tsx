    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-surface flex flex-col items-center justify-center text-[#DDDADB]">
          ...
          <button onClick={() => router.push('/')} className="bg-[#C13535] px-6 py-2 rounded-full font-bold">Volver al inicio</button>
        </div>
      </>
    );
  }
        ...
        <div className="flex justify-between items-center px-4 md:px-6 py-4 w-full max-w-screen-2xl mx-auto">
          <div className="flex items-center gap-4 md:gap-8">
            ...
            <nav className="hidden lg:flex items-center gap-6">
              <button onClick={() => router.push('/')} className="text-[#C13535] dark:text-[#DDDADB] hover:text-[#F07D00] transition-colors font-['Montserrat'] font-bold tracking-tight">Inicio</button>
              ...
              <button onClick={() => router.push('/contacto')} className="text-[#C13535] dark:text-[#DDDADB] hover:text-[#F07D00] transition-colors font-['Montserrat'] font-bold tracking-tight">Contacto</button>
            </nav>
          </div>
          ...
          <div className="relative z-10 w-full max-w-screen-2xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mt-12 md:mt-0">
            <div className="space-y-8 max-w-2xl">
              ...
              <div className="flex flex-wrap gap-3 md:gap-4">
                <button onClick={() => latestVideo && router.push(`/watch/${latestVideo.id}`)} className="bg-[#C13535] text-white px-6 md:px-8 py-3 md:py-4 rounded-full text-sm md:text-base font-bold flex items-center gap-2 md:gap-3 hover:scale-105 transition-all active:scale-95 shadow-[0_0_20px_rgba(193,53,53,0.3)]" disabled={!latestVideo}>
          ...
          {videoEpisodes.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {videoEpisodes.map(video => (
                <div key={video.id} onClick={() => router.push(`/watch/${video.id}`)} className="group relative bg-zinc-100 dark:bg-[#201f20] rounded-xl overflow-hidden transition-all duration-500 hover:scale-[1.02] hover:shadow-[0_20px_40px_rgba(193,53,53,0.15)] cursor-pointer border border-zinc-200 dark:border-transparent">
