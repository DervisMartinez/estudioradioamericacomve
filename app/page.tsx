      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 dark:bg-[#131314]/80 backdrop-blur-xl flex justify-between items-center px-4 md:px-8 h-20 shadow-sm dark:shadow-none border-b border-zinc-200 dark:border-transparent">
        <div className="flex items-center gap-4 md:gap-12">
          ...
          <div className="hidden lg:flex gap-8 items-center font-['Montserrat'] tracking-tight font-bold">
            ...
            <button onClick={() => router.push('/contacto')} className="text-[#C13535] dark:text-[#DDDADB] hover:text-[#F07D00] transition-colors duration-300">Contáctanos</button>
          </div>
        </div>
        ...
        {isMobileMenuOpen && (
          <div className="absolute top-20 left-0 w-full bg-white dark:bg-[#131314] shadow-lg border-b border-zinc-200 dark:border-[#1c1b1c] lg:hidden flex flex-col font-['Montserrat'] tracking-tight font-bold animate-fade-in z-50">
            ...
            <button onClick={() => { router.push('/contacto'); setIsMobileMenuOpen(false); }} className="px-6 py-4 text-left text-[#C13535] dark:text-[#DDDADB]">Contáctanos</button>
          </div>
        )}
      </nav>
          ...
          <div className={`relative z-10 max-w-7xl mx-auto px-6 md:px-8 w-full grid md:grid-cols-2 gap-8 md:gap-12 items-center transition-opacity duration-1000 ${fadeClass}`}>
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4 pt-4">
                <button onClick={() => featuredVideo && router.push(`/watch/${featuredVideo.id}`)} className={`${featuredVideo?.isLive ? 'bg-red-600 hover:bg-red-700 shadow-red-900/20' : 'bg-[#C13535] hover:bg-[#a12b2b] shadow-primary/20'} text-white px-6 md:px-8 py-3 md:py-4 rounded-full text-sm md:text-base font-bold flex items-center gap-2 transition-all transform hover:scale-105 active:scale-95 shadow-lg whitespace-nowrap`}>
                ...
                <PressNoteButton 
                  ...
                  className="bg-zinc-200/50 dark:bg-surface-container/50 backdrop-blur-md text-[#C13535] dark:text-on-surface px-5 py-3 md:py-4 rounded-full text-xs md:text-sm font-bold border border-zinc-300 dark:border-outline-variant/30 hover:bg-zinc-200 dark:hover:bg-surface-container transition-all whitespace-nowrap flex items-center gap-2 shadow-lg" 
                />
              </div>
            </div>

            <div className="hidden md:block relative group cursor-pointer" onClick={() => featuredVideo && router.push(`/watch/${featuredVideo.id}`)}>
              ...
              {nextVideo && (
                <div onClick={(e) => { e.stopPropagation(); router.push(`/watch/${nextVideo.id}`); }} className="absolute -bottom-6 -right-6 bg-white/60 dark:bg-surface-container/60 backdrop-blur-md p-4 rounded-lg border border-zinc-200 dark:border-outline-variant/20 hidden lg:block hover:scale-105 transition-transform cursor-pointer shadow-2xl z-20">
          ...
          <div className="max-w-7xl mx-auto px-8">
            ...
            >
              {programs.map(program => (
                <div key={program.id} onClick={() => router.push(`/program/${program.id}`)} className="group cursor-pointer flex-none w-40 md:w-56 snap-start">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
              ...
              <div className="md:col-span-5 flex flex-col gap-6 border-l border-zinc-200 dark:border-outline-variant/10 md:pl-12">
                ...
                {categoryFeaturedVideo && (
                  <div onClick={() => router.push(`/watch/${categoryFeaturedVideo.id}`)} className="w-full bg-white/80 dark:bg-surface-container/80 backdrop-blur-md rounded-2xl overflow-hidden relative border border-zinc-200 dark:border-outline-variant/10 group cursor-pointer mb-4">
                  ...
                  <div className="max-h-80 overflow-y-auto p-2 custom-scrollbar">
                    {viewHistory.length === 0 ? (
                      ...
                      viewHistory.map(video => (
                        <div key={video.id} onClick={() => router.push(`/watch/${video.id}`)} className="flex items-center gap-3 p-2 hover:bg-zinc-100 dark:hover:bg-surface-container-highest rounded-xl cursor-pointer transition-colors group">
      ...
      <footer className="bg-zinc-100 dark:bg-[#131314] w-full border-t border-zinc-200 dark:border-[#59413f]/15 pt-16 pb-32">
        <div className="max-w-7xl mx-auto px-8 flex flex-col items-center space-y-8">
          ...
          <div className="flex flex-wrap justify-center gap-8 font-['Montserrat'] text-sm text-[#C13535] dark:text-[#DDDADB]">
            ...
            <button onClick={() => router.push('/contacto')} className="text-[#C13535]/80 dark:text-[#DDDADB]/60 hover:text-[#C13535] transition-colors">Contacto</button>
